function solution = Jaccard(filename,filename2)
...
Xa=(X~=0);
Ya=(Y~=0);
solution.Whole=sum(and(Xa,Ya))/sum(or(Xa,Ya));
Xa=(X==1)|(X==3)|(X==4);
Ya=(Y==1)|(Y==3)|(Y==4);
solution.Core=sum(and(Xa,Ya))/sum(or(Xa,Ya));
Xa=(X==4);
Ya=(Y==4);
solution.Active=sum(and(Xa,Ya))/sum(or(Xa,Ya));
